...
const run = async () => {
    const cables = await read('./data.txt', 'utf8').then(data => data.split('\n'));
    ...
    console.log(matchingPoints.map(item => _.sum(item.split(',').map(val => Math.abs(Number(val))))));
}
...
const part2 = async () => {
    ...
    const matchingPoints = _.intersection(points1, points2);
    console.log(matchingPoints);
    const yeah = matchingPoints.map(item => {
        return {
